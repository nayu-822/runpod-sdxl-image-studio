...
- ドキュメントの説明文・見出し・コメントは日本語で作成・更新する。製品名、API 名、クラス名、設定キー、コマンド、コード例などの固有名詞・識別子は原文のまま保持してよい。
- UI と画像生成ロジックを分離する。
...
- ワークフローテンプレートは Git 管理可能な JSON とする。
- 実行時にはテンプレートをコピーして必要項目だけ差し替える。
- ノードクラス、ノード ID、入力キーをテンプレート定義と対応付けによって管理する。
- 任意ノードの追加を UI から許可しない。
- テンプレートには `template_id`、`schema_version`、必要なカスタムノードを定義する。
- テンプレート変更時には互換性テストを追加する。
- JSON の直接文字列置換は禁止する。
- 生成前に必須ノードと入力を検証する。
...
- HTTP のタイムアウトと WebSocket のタイムアウトを設定する。
- 再試行は冪等性を考慮する。
- `/prompt` の二重送信を避ける。
- prompt ID を永続化してから監視を開始する。
- WebSocket 切断時は履歴 API で状態を確認する。
- キャンセル操作は ComfyUI のキュー操作とアプリ DB 状態を整合させる。
...
- 元画像を上書きしない。
- 親画像と派生画像を別レコードにする。
- 主 `image` Artifact の保存・登録、Generation 完了、Job 完了は同一 SQLite トランザクションで確定する。
- 主 `image` Artifact または完了状態の DB 保存に失敗した場合は成功を返さず、DB をロールバックし、画像ファイルを削除しない。
- metadata JSON と thumbnail は補助成果物として扱い、失敗しても主画像と Generation / Job の成功を取り消さない。
- 補助成果物の保存は冪等にし、復旧時に sidecar・thumbnail を重複作成しない。
- 不完全保存は後処理または復旧対象として記録する。
- サムネイルは原寸画像と別管理する。
...
- タブ・セクション単位で UI 構築処理を分離する。
- UI ハンドラーは入力変換、検証、サービス呼び出し、表示モデル変換に限定する。
- 画像生成を同期コールバック内で完了まで待たない設計を優先する。
- モバイル幅で横スクロールが発生しないことを確認する。
- 高度な設定は Accordion を使用する。
- 主要ボタンは十分な高さと間隔を持たせる。
- Gallery の大量一括読込を避ける。
- 選択中 generation ID を明示的に状態管理する。
- 最近使った設定や最近使った LoRA は別の表示モデルで管理する。
- 再生成ボタンは処理開始前に無効化し、復元失敗・入力検証失敗・成功・失敗の全経路で復帰させる。
- 復元処理の失敗は後続の再生成イベントへ進めない。
- checkpoint、VAE、LoRA の候補 `None` は未取得、空の候補は取得済みで0件として扱う。
- 候補が未取得の場合はモデル存在確認を行わず、利用者へ明示する。
...
- エラーは再試行可能 / 再試行不可を区別する。
- checkpoint 不足、LoRA 不足、カスタムノード不足、GDrive 失敗を別エラー型とする。
- 失敗した generation も監査可能な状態で記録する。

## 15. ログ

- 構造化ログを推奨する。
- generation ID、job ID、prompt ID をコンテキストに含める。
- prompt は個人情報を含む可能性があるため、通常ログへ全文を重複出力しない。
- API キー、rclone config、Cookie を出力しない。
- ComfyUI response を無制限にログ出力しない。
- サブプロセスのログは専用ファイルへ保存する。

## 16. テスト

必須ユニットテスト:

- `GenerationSettings` の検証
- LoRA 重複・強度の検証
- ワークフローテンプレートの対応付け
- metadata のシリアライズ / デシリアライズ
- PNG metadata 不在時の sidecar へのフォールバック
- generation の親子関係
- 日付フォルダーの算出
- 安全なパス解決
- rclone コマンドの組み立て
