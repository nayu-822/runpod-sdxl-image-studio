...
sidecarがmalformed JSON、invalid UTF-8、unsupported schemaの場合もcanonical画像とMetadataImport行は保持し、raw本文または安全なhashを保存します。metadata statusは実行不可となり、画像upscaleだけを許可し、latent upscaleと生成条件復元は拒否します。保存先はdata rootへresolveし、既存parentのsymlinkがroot外へ解決する場合は書込み前に拒否します。
...
source選択の候補と確認状態は `0011_phase6_metadata_source_selection` で保存し、0010/0011時点のambiguous rowは `0012_phase6_legacy_metadata_candidates` でraw sourceから候補を再構築します。自動選択は行わず、raw metadataを保持します。通常のdowngradeでは0012/0011を戻せますが、外部upscale行が残る状態で0010から0009へ戻す操作は安全のため拒否されます。completed外部upscaleの比較表示は `source_import_id` のcanonical画像を再検証して使用します。
